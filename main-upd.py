...
def neighbors_find(lbl_img, im_label,bboxx,bboxy, pixeldistance):
    ...
    import cv2
    import time
    #idx = np.where((lbl_img >=1) & (lbl_img <= 10))
    #idx = (lbl_img<=1)*(lbl_img<=1297)
    # idx1 = np.where(idx)
    # contours =cv2.findContours(lbl_img.astype('uint8'),cv2.RETR_CCOMP, cv2.CHAIN_APPROX_TC89_KCOS)[0]
    # for contour in contours:
    #     x,y,w,h = cv2.boundingRect(contour)
    #     ROI = lbl_img[y:y+h,x:x+w]
    
    
    # idx = cv2.findContours(lbl_img.astype('uint8'),cv2.RETR_CCOMP, cv2.CHAIN_APPROX_TC89_KCOS)[1][0]
    # out = np.zeros_like(lbl_img)
    # out[idx[0,0],idx[:,1]] = 255


    # out = np.zeros_like(aa)#   .squeeze()
    # out[contours[:, 0], contours[:, 1]] = 1
                    # from scipy.sparse import csr_matrix

                    # # def compute_M(data):
                    # cols = np.arange(lbl_img.size)
                    # M = csr_matrix((cols, (lbl_img.ravel(), cols)),
                    #                     shape=(lbl_img.max() + 1,lbl_img.size))

                    # # def get_indices_sparse(data):
                    # #     M = compute_M(data)
                    # check1 = [np.unravel_index(row.data, lbl_img.shape) for row in M]

    nz = np.nonzero(lbl_img)
    coords = np.column_stack(nz)
    nzvals = lbl_img[nz[0], nz[1]]
    contours = [coords[nzvals==k] for k in range(1, len(im_label)+ 1)]
    #contours = np.where((nzvals >=1) & (nzvals<= 1297))
    for contour in contours:
        #ck= [(contour[i][j]) for i in range(0,len(contour)) for j in  range(0,len(contour[i])) ]

        pxx = [x[1] for x in contour]
        pyy = [x[0] for x in contour]
...
def feature_extraction(features,
    ...
    def orientation(seg_img,*args):
        ...
        data_dict=[]
        # xx= np.where(np.logical_and(seg_img>=1, seg_img<=1298))
        # xg=xx[0].mean()
        # yg=xx[1].mean()
        # x = xx[0] - xg
        ...
        # uyy = (x**2).sum()
        # uxy = (x*y).sum()
        # #for uxx,uyy,uxy in zip(uxx1,uyy1,uxy1):
        # if (uyy > uxx):
        ...
        # print(data_dict)
        # print(yyy)
        idx = np.where((lbl_img >=1) & (lbl_img <= 1297))
        x=idx[0]
        y=idx[1]
    ...
    def bbox_ymin(*args):
        ...
        logger.info('Completed extracting boundingbox_ymin for ' + seg_file_names1.name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def bbox_xmin(*args):
        ...
        logger.info('Completed extracting boundingbox_xmin for ' + seg_file_names1.name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def bbox_width(*args):
        ...
        logger.info('Completed extracting boundingbox_width for ' + seg_file_names1.name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def bbox_height(*args):
        ...
        logger.info('Completed extracting boundingbox_height for ' + seg_file_names1.name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def mean_intensity(*args):
        ...
        logger.info('Completed extracting mean intensity for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def max_intensity(*args):
        ...
        logger.info('Completed extracting maximum intensity for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def min_intensity(*args):
        ...
        logger.info('Completed extracting minimum intensity for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def median(*args):
        ...
        logger.info('Completed extracting median for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def mode(*args):
        ...
        logger.info('Completed extracting mode for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def standard_deviation(*args):
        ...
        logger.info('Completed extracting standard deviation for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def skewness(*args):
        ...
        logger.info('Completed extracting skewness for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def entropy(*args):
        ...
        logger.info('Completed extracting entropy for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def kurtosis(*args):
        ...
        logger.info('Completed extracting kurtosis for ' + int_file_name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        return data_dict
    ...
    def neighbors(seg_img, *args):
        ...
        bbox_value = [str(region.bbox) for region in regions]
        bbox_all = [value.split(',') for value in bbox_value]
        bbox_all1 = [bbox_min[0].replace('(','') for bbox_min in bbox_all]
        bbox_all2 = [bbox_min[1].replace(' ','') for bbox_min in bbox_all]
        # bboxx = [bbox_min[1] for bbox_min in bbox_all2]
        # bboxy = [bbox_min[0] for bbox_min in bbox_all1]
        # print(bbox_all2)
        # print(yyy)
        data_dict= neighbors_find(seg_img, label,bbox_all1,bbox_all2, pixelDistance)
    ...
    def all(seg_img, units, int_img):
        ...
        tic = time.process_time()
        all_hexagonality_score = [poly[1] for poly in all_polygon_score]#hexagonality_score(seg_img, units)
        logger.info('Completed extracting hexagonality score for ' + seg_file_names1.name)
        toc = time.process_time()
        print("Computation time = " + str(1000*(toc - tic )) + "ms")
        #calculate hexagonality standarddeviation
        all_hexagonality_sd = [poly[2] for poly in all_polygon_score]#hexagonality_sd(seg_img, units)
...
def main():
    ...
    # intDir = 'C:/Users/nagarajanj2/Downloads/mipint'
    # segDir = 'C:/Users/nagarajanj2/Downloads/miplab'
    ...
    features = ['neighbors']
    csvfile= 'singlecsv'
    pattern = '.*.ome'#'image_c000_z00{z}_ch{c}'
...
if __name__ == "__main__":
    main()
        ...
        # ck=[(contour[0][j])for j in range(0,len(contour[0]))]
        # pxx = [x[1] for x in ck]
        # pyy = [x[0] for x in ck]
        # pypdd = [int(py-pixeldistance) for py in pyy]
        # pxpdd = [int(px-pixeldistance) for px in pxx]
        # pypd_rnn=[int(py+pixeldistance) for py in pyy]
        # pxpd_rnn=[int(px+pixeldistance) for px in pxx]
        # cand1=[(p,q) for p in (range(pypd,pypd_rn+1) for pypd,pypd_rn in zip(pypdd,pypd_rnn)) for q in(range(pxpd,pxpd_rn+1) for pxpd,pxpd_rn in zip(pxpdd,pxpd_rnn))]
        # cand = [(m,n) for m in cand1[0] for n in cand1[1]]
        # for e,f in cand1:
        #     for l in e :
        #         for k in f: